...
const Transaction = () => {
  ...
  const fetchTransactions = async () => {
    try {
      const res = await axios.get("https://backend-production-5264.up.railway.app/transaction");
      if (res.data.success) {
  ...
  const handlePay = async (id) => {
    try {
      const res = await axios.post(`https://backend-production-5264.up.railway.app/transaction/pay/${id}`);
      if (res.data.success) {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await axios.delete(`https://backend-production-5264.up.railway.app/transaction/${id}`);
      if (res.data.success) {
